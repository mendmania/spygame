...
/**
 * Admin Index Update Actions for Spyfall
 * 
...
export async function updateAdminRoomIndex(
  ...
  try {
    ...
    const indexRef = db.ref(`${ADMIN_INDEX_BASE}/rooms/spyfall/${roomId}`);
...
export async function updateAdminRoomStatus(
  ...
  try {
    ...
    const wasActive = currentData.status === 'waiting' || currentData.status === 'playing';
...
export async function updateAdminPlayerCount(
  ...
  try {
    ...
    const isActive = currentData.status === 'waiting' || currentData.status === 'playing';
